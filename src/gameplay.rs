...
/// upgrade. (The buff tether node is a boss-fight mechanic that enables boss
/// damage and no longer boosts momentum.) Early zones are slower and it ramps up
/// gradually as the player progresses (via the zone multiplier), so the fast
/// ceiling is a late-game reward. Used to scale spawn-animation speed / lookahead
/// so fast players don't outpace procedural generation.
pub fn player_max_momentum(s: &State) -> f32 {
    if s.in_space_mode {
        SPACE_MOMENTUM_CAP
    } else {
        let zone_mult = match zone_index_for_distance(s.distance) {
            0 => 1.0,
            1 => 1.18,
            _ => 1.36,
        };
