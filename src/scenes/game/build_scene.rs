...
fn hook_base_for_zone(zone_idx: usize) -> (u8, u8, u8) {
    match zone_idx {
        1 => C_HOOK_ZONE1,
        2 => C_HOOK_ZONE2,
        _ => C_HOOK,
...
fn hook_near_for_zone(zone_idx: usize) -> (u8, u8, u8) {
    match zone_idx {
        1 => C_HOOK_NEAR_ZONE1,
        2 => C_HOOK_NEAR_ZONE2,
        _ => C_HOOK_NEAR,
...
fn hook_on_for_zone(zone_idx: usize) -> (u8, u8, u8) {
    match zone_idx {
        1 => C_HOOK_ON_ZONE1,
        2 => C_HOOK_ON_ZONE2,
        _ => C_HOOK_ON,
...
fn pad_for_zone(zone_idx: usize) -> (u8, u8, u8) {
    match zone_idx {
        1 => C_PAD_ZONE1,
        2 => C_PAD_ZONE2,
        _ => C_PAD,
...
fn pad_hit_for_zone(zone_idx: usize) -> (u8, u8, u8) {
    match zone_idx {
        1 => C_PAD_HIT_ZONE1,
        2 => C_PAD_HIT_ZONE2,
        _ => C_PAD_HIT,
...
fn spinner_for_zone(zone_idx: usize) -> (u8, u8, u8) {
    match zone_idx {
        1 => C_SPINNER_ZONE1,
        2 => C_SPINNER_ZONE2,
        _ => C_SPINNER,
...
pub fn build_game_scene(ctx: &mut Context) -> Scene {
    ...
    let bg_zone_start_vivid = gradient_rect(4, VH as u32, (8, 26, 74), (104, 194, 255));
    let bg_zone_purple_vivid = gradient_rect(4, VH as u32, (56, 18, 94), (165, 78, 230));
    let bg_zone_black_vivid = gradient_rect(4, VH as u32, (212, 142, 28), (255, 236, 120));
    ...
    scene.on_enter(move |canvas| {
        ...
        if !pause_key_registered {
            canvas.on_key_press(|c, key| {
                ...
                if *key == Key::Character("1".into()) {
                    let vivid_now = matches!(c.get_var("bg_vivid"), Some(Value::Bool(true)));
                ...
                let game_paused = c.is_paused() || matches!(c.get_var("game_paused"), Some(Value::Bool(true)));
                if game_paused {
                    c.resume();
        ...
        canvas.register_custom_event("do_release".into(), move |c| {
            ...
            let zone_idx_now = zone_index_for_distance(s.distance);
            ...
            if !prev.is_empty() {
                if let Some(obj) = c.get_game_object_mut(&prev) {
                    let (r, g, b) = hook_base_for_zone(zone_idx_now);
                    obj.set_image(Image {
                        shape: ShapeType::Ellipse(0.0, (HOOK_R*2.0, HOOK_R*2.0), 0.0),
                        image: circle_img(HOOK_R as u32, r, g, b).into(),
                        color: None,
            ...
            if let Some((hook_id, hx, hy, dist)) = nearest {
                ...
                apply_grab_impulse(&mut s, hx, hy);
                    ...
                    if let Some(obj) = c.get_game_object_mut(&hook_id) {
                        let (r, g, b) = hook_on_for_zone(zone_idx_now);
                        obj.set_image(Image {
                            shape: ShapeType::Ellipse(0.0, (HOOK_R*2.0, HOOK_R*2.0), 0.0),
                            image: circle_img(HOOK_R as u32, r, g, b).into(),
                            color: None,
        ...
        let mut prev_palette_zone: Option<usize> = None;
        ...
        let bg_zone_start_vivid_img = bg_zone_start_vivid.clone();
        let bg_zone_purple_vivid_img = bg_zone_purple_vivid.clone();
        let bg_zone_black_vivid_img = bg_zone_black_vivid.clone();
        canvas.on_update(move |c| {
            ...
            // Keep screen-anchored world backdrops locked to the camera even
            // while pause animation is running, otherwise fast momentum can
            // reveal the clear color at the viewport edges.
            let cam_x_now = c.camera().map(|cam| cam.position.0).unwrap_or(0.0);
            if let Some(obj) = c.get_game_object_mut("bg") {
                obj.position.0 = cam_x_now;
            }
            let floor_y_now = {
                let s = st.lock().unwrap();
            ...
            if let Some(obj) = c.get_game_object_mut("danger_floor") {
                obj.position = (cam_x_now, floor_y_now);
            }
            ...
            if c.is_paused() || matches!(c.get_var("game_paused"), Some(Value::Bool(true))) {
                if let Some(obj) = c.get_game_object_mut("pause_overlay") {
                    obj.position.0 = cam_x_now;
                }
                return;
            ...
            // longer and slightly wider connected ribbon.
            {
                ...
                // Emit opposite to current movement so the trail stays behind.
                let evx = (-s.vx * 0.55).clamp(-34.0, 34.0);
                let evy = (-s.vy * 0.55).clamp(-34.0, 34.0);
                drop(s);
            ...
            let pad_reset_zone = zone_index_for_distance(s.distance);
            drop(s);
            for id in &expired_glows {
                if let Some(obj) = c.get_game_object_mut(id) {
                    ...
                    } else if id.starts_with("pad_") {
                        let (pr, pg, pb) = pad_for_zone(pad_reset_zone);
                        obj.set_image(Image {
            ...
            let hook_live = s.live_hooks.clone();
            let active_hook_now = s.active_hook.clone();
            let spinner_vertical_active = zone_idx >= 2;
            let spinner_t = s.ticks as f32 / 60.0;
            drop(s);

            if prev_palette_zone != Some(zone_idx) {
                let (pr, pg, pb) = pad_for_zone(zone_idx);
                let (sr, sg, sb) = spinner_for_zone(zone_idx);
                let (hb_r, hb_g, hb_b) = hook_base_for_zone(zone_idx);
                let (hn_r, hn_g, hn_b) = hook_near_for_zone(zone_idx);
                let (ho_r, ho_g, ho_b) = hook_on_for_zone(zone_idx);

                for name in &spinner_live {
                    if let Some(obj) = c.get_game_object_mut(name) {
                        obj.set_image(Image {
                            shape: ShapeType::Rectangle(0.0, (SPINNER_W, SPINNER_H), 0.0),
                            image: spinner_img(SPINNER_W as u32, SPINNER_H as u32, sr, sg, sb).into(),
                            color: None,
                        });
                    }
                }
                ...
                for name in &hook_live {
                    if let Some(obj) = c.get_game_object_mut(name) {
                        let (r, g, b) = if *name == active_hook_now {
                            (ho_r, ho_g, ho_b)
                        } else if *name == prev_nearest_hook {
                            (hn_r, hn_g, hn_b)
                        } else {
                            (hb_r, hb_g, hb_b)
                        };
                ...
                prev_palette_zone = Some(zone_idx);
            }
                ...
                && !s.pool_free.is_empty()
            {
                if let Some(spec) = s.pending.pop_front() {
                    ...
                    let zone_idx_spawn = zone_index_for_distance(s.distance);
                    hooks_spawned_this_tick += 1;
                    drop(s);

                    if let Some(obj) = c.get_game_object_mut(&id) {
                        let (r, g, b) = hook_base_for_zone(zone_idx_spawn);
                ...
                && !s.pad_free.is_empty()
            {
                ...
                if let Some(obj) = c.get_game_object_mut(&id) {
                    let (pr, pg, pb) = pad_for_zone(zone_idx_spawn);
            ...
            if s.bounce_enabled && (falling_down || falling_up) {
                ...
                if let Some(pad_name) = bounced_pad {
                    ...
                    if s.hooked {
                        ...
                        if !prev.is_empty() {
                            if let Some(obj) = c.get_game_object_mut(&prev) {
                                let (hr, hg, hb) = hook_base_for_zone(zone_idx_now);
                                obj.set_image(Image {
                                    shape: ShapeType::Ellipse(0.0, (HOOK_R*2.0, HOOK_R*2.0), 0.0),
                                    image: circle_img(HOOK_R as u32, hr, hg, hb).into(),
                                    color: None,
                                });
                            }
                        }
                        ...
                        if let Some(obj) = c.get_game_object_mut(&pad_name) {
                            let (pr, pg, pb) = pad_hit_for_zone(zone_idx_now);
                            obj.set_image(Image {
                                shape: ShapeType::Rectangle(0.0, (PAD_W, PAD_H), 0.0),
                                image: pad_img(PAD_W as u32, PAD_H as u32, pr, pg, pb).into(),
                                color: None,
            ...
            if let Some(obj) = c.get_game_object_mut("bg") {
                ...
                let bg_theme = (dark_now, zone_idx, vivid_now);
                if prev_bg_theme != Some(bg_theme) {
                    let image_data = if dark_now {
                        solid(4, 4, 8, 255)
                    } else {
                        match (vivid_now, zone_idx) {
                            (false, 0) => bg_zone_start_img.clone(),
                            (false, 1) => bg_zone_purple_img.clone(),
                            (false, _) => bg_zone_black_img.clone(),
                            (true, 0) => bg_zone_start_vivid_img.clone(),
                            (true, 1) => bg_zone_purple_vivid_img.clone(),
                            (true, _) => bg_zone_black_vivid_img.clone(),
                        }
                    };
                    obj.set_image(Image {
                        shape: ShapeType::Rectangle(0.0, (VW, VH), 0.0),
            ...
            // ── Highlight nearest grabbable hook ──────────────────────────────
            {
                ...
                if cur_nearest != prev_nearest_hook {
                    let zone_idx_now = {
                        let s = st.lock().unwrap();
                        zone_index_for_distance(s.distance)
                    };
                    ...
                    if !cur_nearest.is_empty() && cur_nearest != active {
                        if let Some(obj) = c.get_game_object_mut(&cur_nearest) {
                            let (r, g, b) = hook_near_for_zone(zone_idx_now);
                            obj.set_image(Image {
                                shape: ShapeType::Ellipse(0.0, (HOOK_R*2.0, HOOK_R*2.0), 0.0),
                                image: circle_img(HOOK_R as u32, r, g, b).into(),
                                color: None,
