...
//   • Rain:        full-screen rain overlay with directional bias.
//   • Droplets:    screen-space water droplets on glass.
...
const PLAYER_EFFECT_COUNT: usize = 8;
...
// ── Comet state (background screen-space objects) ────────────────────────────
//
...
///
/// Uses CRYSTALLINE-CORRECTED position from the engine object for player overlay
/// alignment. world_to_screen() ensures perfect camera synchronization.
///
/// Parameters:
/// - `c` – Canvas (has camera, world_to_screen transform, and mega shader stack)
/// - `st` – Game state Arc (for momentum and VFX timers)
/// - `ball_img` – white circle sprite for player + triggered VFX
/// - `comet_img` – orange circle sprite for comet head/tail sprites
/// - `comets` – mutable comet states (screen-pixel space, camera-independent)
...
/// - `player_effect_idx` – effect cycling index (0-7)
pub fn tick_mega_shaders(
    ...
    let pad_timer: i32 = 0;
    let pad_pos: (f32, f32) = (0.0, 0.0);
    let spin_timer: i32 = 0;
    let spin_pos: (f32, f32) = (0.0, 0.0);
    {
    ...
    // This is the SAME position used in the scene render pass, so overlays
    // and game objects are perfectly aligned. No one-frame lag.
    ...
    let shader_airshield_on = matches!(
        c.get_var("fx_shader_airshield_on"),
        Some(Value::Bool(true))
    );
    let fx_comets_on = matches!(c.get_var("fx_comets_on"), Some(Value::Bool(true)));
    ...
    let effect_idx_raw = match c.get_var("player_effect_idx") {
        Some(Value::I32(v)) => v.max(0),
        _ => 0,
    };
    let effect_idx = (effect_idx_raw as usize)
    ...
    if shader_airshield_on && speed >= air_threshold {
