...
fn tick_pad_thrusters(c: &mut Canvas, st: &Arc<Mutex<State>>) {
    let (pad_ids, gravity_dir) = {
        let s = st.lock().unwrap();
        (s.pad_live.clone(), s.gravity_dir)
    };
    ...
    for pad_id in &pad_ids {
        ...
        if let Some(thr) = c.get_game_object_mut(&thr_id) {
            ...
            thr.position.1 = if gravity_dir < 0.0 {
                py - PAD_THRUSTER_H + PAD_THRUSTER_HIDE_TOP + PAD_THRUSTER_RAISE_Y
            } else {
                py + PAD_H - PAD_THRUSTER_HIDE_TOP - PAD_THRUSTER_RAISE_Y
            };
            thr.layer = layer - 1;
            thr.visible = vis;
        }
